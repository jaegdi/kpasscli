...
if ./kpasscli -h > /dev/null; then
    ...
    artifactory-upload.sh -lf=kpasscli       -tr=scpas-bin-dev-local      -tf=/kpasscli/kpasscli-linux/

    artifactory-upload.sh -lf=kpasscli.exe   -tr=scptools-bin-dev-local   -tf=/tools/kpasscli/kpasscli-windows/
    artifactory-upload.sh -lf=kpasscli.exe   -tr=scpas-bin-dev-local      -tf=/kpasscli/kpasscli-windows/
